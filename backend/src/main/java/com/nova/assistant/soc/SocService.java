...
/**
 * SOC analyst toolkit (no AI dependency): IOC extraction, defang/refang,
 * multi-format decoding, CVE enrichment (NVD) and VirusTotal reputation.
 */
@Service
...
public class SocService {
    ...
    static final Pattern SHA256 = Pattern.compile("\\b[a-fA-F0-9]{64}\\b");
    static final Pattern SHA1 = Pattern.compile("\\b[a-fA-F0-9]{40}\\b");
    static final Pattern MD5 = Pattern.compile("\\b[a-fA-F0-9]{32}\\b");
    static final Pattern CVE = Pattern.compile("(?i)CVE-\\d{4}-\\d{4,7}");
    ...
    public IocResult extractIocs(String raw) {
        ...
        for (String d : findAll(DOMAIN, stripped)) {
            ...
            if (FILE_EXT.contains(tld)) continue;
    ...
    private List<String> dedupe(List<String> in) { return new ArrayList<>(new LinkedHashSet<>(in)); }
    ...
    public DecodeResult decode(String input, String mode) {
        ...
        try {
            return switch (m) {
                case "base64" -> new DecodeResult(m, new String(Base64.getMimeDecoder().decode(input.trim()), StandardCharsets.UTF_8), true);
    ...
    private String pad(String b64) { int m = b64.length() % 4; return m == 0 ? b64 : b64 + "====".substring(m); }
    ...
    public CveResult cveLookup(String rawId) {
        ...
        try {
            ...
            if (descs != null) for (Map<String, Object> d : descs)
                if ("en".equals(d.get("lang"))) { description = String.valueOf(d.get("value")); break; }
            Double score = null; String severity = null;
            ...
            if (references != null) for (Map<String, Object> r : references) {
                Object u = r.get("url"); if (u != null && refs.size() < 6) refs.add(String.valueOf(u));
            }
            ...
            return new CveResult(id, null, null, null, null, List.of(), false, "No se pudo consultar la NVD (limite de tasa o sin conectividad).");
        }
    }
    private CveResult notFound(String id) { return new CveResult(id, null, null, null, null, List.of(), false, "CVE no encontrado en la NVD"); }

    public VtResult enrichVt(String rawIndicator) {
        String key = properties.getSoc().getVirustotal().getApiKey();
        if (key == null || key.isBlank())
            return new VtResult(rawIndicator, "?", null, 0, 0, 0, 0, null, null, null, false, "VirusTotal no configurado (define VIRUSTOTAL_API_KEY).");
        String indicator = refang(rawIndicator).trim();
        String type, path, guiType, guiId;
        if (IPV4.matcher(indicator).matches()) { type = "ip"; path = "/ip_addresses/" + indicator; guiType = "ip-address"; guiId = indicator; }
        else if (SHA256.matcher(indicator).matches() || SHA1.matcher(indicator).matches() || MD5.matcher(indicator).matches()) { type = "file"; path = "/files/" + indicator; guiType = "file"; guiId = indicator; }
        else if (URL.matcher(indicator).find()) { String vid = Base64.getUrlEncoder().withoutPadding().encodeToString(indicator.getBytes(StandardCharsets.UTF_8)); type = "url"; path = "/urls/" + vid; guiType = "url"; guiId = vid; }
        else if (DOMAIN.matcher(indicator).find()) { type = "domain"; path = "/domains/" + indicator; guiType = "domain"; guiId = indicator; }
        else return new VtResult(indicator, "?", null, 0, 0, 0, 0, null, null, null, false, "Tipo de indicador no reconocido (IP, dominio, URL o hash).");
        String link = "https://www.virustotal.com/gui/" + guiType + "/" + guiId;
        try {
            RestClient client = restClientBuilder.build();
            @SuppressWarnings("unchecked")
            Map<String, Object> body = client.get().uri("https://www.virustotal.com/api/v3" + path).header("x-apikey", key).retrieve().body(Map.class);
            if (body == null || body.get("data") == null) return new VtResult(indicator, type, null, 0, 0, 0, 0, null, null, link, false, "Sin datos en VirusTotal.");
            @SuppressWarnings("unchecked") Map<String, Object> data = (Map<String, Object>) body.get("data");
            @SuppressWarnings("unchecked") Map<String, Object> attr = (Map<String, Object>) data.get("attributes");
            @SuppressWarnings("unchecked") Map<String, Object> stats = attr == null ? null : (Map<String, Object>) attr.get("last_analysis_stats");
            ...
            return new VtResult(indicator, type, verdict, mal, susp, harm, undet, reputation, buildVtDetails(type, attr), link, true, null);
        } catch (Exception e) {
            log.warn("VirusTotal lookup failed for {}: {}", indicator, e.getMessage());
            String msg = e.getMessage() != null && e.getMessage().contains("404") ? "No encontrado en VirusTotal." : "Error consultando VirusTotal (limite 4/min - 500/dia, o conectividad).";
            return new VtResult(indicator, type, null, 0, 0, 0, 0, null, null, link, false, msg);
        }
    }
    private int num(Object o) { if (o == null) return 0; try { return (int) Double.parseDouble(String.valueOf(o)); } catch (Exception e) { return 0; } }
    ...
    private void appendKv(StringBuilder sb, String label, Object val) {
        if (val != null && !String.valueOf(val).isBlank()) sb.append(label).append(": ").append(val).append("   ");
    }
}
